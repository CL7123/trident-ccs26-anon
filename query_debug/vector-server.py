...
class MemoryMappedOptimizedServer:
    ...
    def _record_memory(self, stage_name: str):
        ...
        try:
            ...
            print(f"[Server {self.server_id} 内存] {stage_name}: {memory_record['rss_mb']:.2f} MB")
        except Exception as e:
            print(f"Server {self.server_id} 内存记录错误: {e}")
    ...
    def _handle_vector_node_query(self, request: Dict) -> Dict:
        ...
        try:
            ...
            self._record_memory("阶段1 VDPF评估完成")
            ...
            self._record_memory("阶段2 e/f计算完成")
            ...
            self._record_memory("阶段3 数据交换完成")
            ...
            self._record_memory("阶段4 重构计算完成")
            ...
            if memory_summary:
                print(f"[Server {self.server_id}] === 内存使用摘要 ===")
                print(f"  基准内存: {memory_summary['baseline_mb']:.2f} MB")
                print(f"  峰值内存: {memory_summary['peak_mb']:.2f} MB")
                print(f"  内存增长: {memory_summary['increase_mb']:.2f} MB")
