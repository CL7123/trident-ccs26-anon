#!/usr/bin/env python3
"""
Optimized distributed server - Phase 2 and Phase 4 using multi-process parallelism
"""

import sys # Add project paths
import os
import socket
import json
import numpy as np
...
try:
    sys.path.append('~/trident/query-opti')
    from cpu_affinity_optimizer import set_process_affinity  # CPU affinity optimization
    total_cores = cpu_count()

    # Each server has its own independent 32 physical cores
    CPU_AFFINITY_AVAILABLE = True
except Exception as e:
    print(f"CPU affinity setting unavailable: {e}")
...
# ==================== Phase 4 Parallel Reconstruction Function ====================
...
class OptimizedDistributedServer:
    ...
    def _load_data(self):
        ...
        # Pre-load triples
        logger.info(f"Triples automatically loaded from local directory")
    ...
    def process_query(self, dpf_key: bytes, query_id: str = None) -> Dict:
        """Handle query request - optimized version"""
        try:
            ...
            # ========== Phase 1: VDPF evaluation (maintain original multi-process) ==========
            ...
            # ========== Phase 2: e/f computation (multi-process parallel) ==========
            logger.info(f"Phase 2: Multi-process e/f computation ({self.phase2_processes} processes)...")
            ...
            # ========== Phase 3: Data exchange (maintain original implementation) ==========
            ...
            # ========== Phase 4: Reconstruction computation (multi-process parallel) ==========
            logger.info(f"Phase 4: Multi-process reconstruction computation ({self.phase4_processes} processes)...")
    ...
    def _handle_client(self, client_socket, client_address):
        ...
        try:
            while True:
                ...
                # Parse request
                try:
                    # Check if it is binary protocol (determined by first byte)
    ...
    def start(self):
        ...
        logger.info(f"Number of Phase 2 processes: {self.phase2_processes}")
        logger.info(f"Number of Phase 4 processes: {self.phase4_processes}")
        ...
        logger.info(f"Process pool warmup complete, elapsed time {warmup_time:.2f}s")
...
# Copy necessary helper functions from original file
def warmup_process(process_id): # Warm up process
    ...
    # Simple calculation task for warmup
...
def vdpf_evaluate_range_optimized(args):
    ...
    # Temporarily skipped, as it requires correct function signature
