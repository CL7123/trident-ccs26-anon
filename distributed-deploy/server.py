...
def warmup_process(process_id):
    """Warmup process to load necessary modules"""
    import time
    import sys
    sys.path.append('~/trident/query-opti')
    from dpf_wrapper_optimized import OptimizedVDPFVectorWrapper
...
def evaluate_batch_range_process(args):
    ...
    try:
        ...
        # Each server has its own independent 64 cores, use process_id as core ID directly
        if process_id < total_cores:
            ...
            # If number of processes exceeds number of cores, use round-robin allocation
        ...
        print(f"[Process {process_id}] CPU affinity binding failed: {e}")
...
class DistributedServer:
    ...
    def __init__(self, server_id: int, dataset: str = "siftsmall", vdpf_processes: int = 32):
        ...
        # Dynamically adjust batch size based on number of processes for better parallelism
        ...
        self.server_config = SERVER_TO_SERVER  # Use private network IP for server-to-server communication
        ...
        logger.info(f"Distributed server initialized")
    ...
    def _warmup_process_pool(self):
        ...
        logger.info(f"Process pool warmup completed in {warmup_time:.2f}s")
        ...
        try:
            while True:
                ...
                if request.get('command') == 'binary_exchange_data':
                    ...
                    logger.debug(f"  Expected uint64 elements: e={len(e_data)//8}, f={len(f_data)//8}")
                    logger.debug(f"  Elements needed for f_shape: {np.prod(f_shape)}")
                    ...
                    logger.info(f"Data reception completed:")
                    logger.info(f"  Source: server {from_server}")
                    logger.info(f"  Duration: {receive_duration:.3f}s")
    ...
    def _send_binary_exchange_data(self, target_server_id: int, query_id: str, e_shares: np.ndarray, f_shares: np.ndarray) -> bool:
        ...
        Phase 1 optimization: use temporary connections instead of shared connections
        - Create independent socket connection for each query
        - Avoid multi-thread contention on the same socket
        - Close connection immediately after sending completes
        """
        ...
        try:
            ...
            logger.info(f"Data transfer completed:")
            logger.info(f"  Duration: {send_duration:.3f}s")
            ...
            # Attempt to rebuild connection and retry once
            logger.info(f"Attempting to rebuild connection to server {target_server_id}...")
            try:
                server_info = self.server_config[target_server_id]
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                ...
                sock.connect((server_info['host'], server_info['port']))
                self.server_connections[target_server_id] = sock
                logger.info(f"Connection rebuilt successfully, retrying send...")

                # Prepare and send data again
                metadata = {
                    'command': 'binary_exchange_data',
                    'query_id': query_id,
                    'from_server': self.server_id,
                ...
                logger.error(f"Connection rebuild and retry failed: {retry_error}")
                return False
    ...
    def _exchange_data_with_servers(self, query_id: str, e_shares: np.ndarray, f_shares: np.ndarray) -> tuple:
        ...
        while time.time() - start_time < max_wait:
            # Check if all data from other servers has been received
            received_all = True
    ...
    def _load_other_servers_data(self, query_id: str, num_nodes: int) -> Tuple[Dict, Dict]:
        """Load data from other servers - enhanced fault tolerance"""
        all_e_from_others = {}
        all_f_from_others = {}
        ...
        for other_id in [1, 2, 3]:
            ...
            # Increase wait time and implement retry mechanism
    ...
    def _handle_vector_node_query(self, request: Dict) -> Dict:
        ...
        try:
            ...
            logger.info(f"Processing vector-level node query, query ID: {query_id}")
            ...
            # Exchange data with other servers over the network
    ...
    def _cleanup_query_files(self, query_id: str):
        ...
        # Clean up in-memory exchange data
